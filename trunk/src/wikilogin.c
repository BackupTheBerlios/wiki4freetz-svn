...
hash(char *s)
{
  ...
  {
    /* basic crypting */
    hashval1 = (hashval1<<4) ^ (hashval1>>28) ^ (int)*s;
    hashval2 = (33*hashval2) + (int)*s;
    s++;
  }
  hashval1 = hashval1 ^ (hashval1>>10) ^ (hashval1>>20);
...
wikilogin_getpermission(char *ipsrc)
{
  ...
  if ( hostlogin )
  /* this computer (localhost is automatically autorized)*/
  {
      ...
      switch( pid )
      {
        case 0:
            ...
            if(execl("/bin/sh","sh","-c",sendmailcmd, NULL) == -1)
            {
              ...
              return -40; //no login yet (user must return with access)
