...
static char* PageFooter = PAGEFOOTER;
...
static char* EditPreview = EDITPREVIEW;
...
int
wiki_show_page(HttpResponse *res, char *wikitext, char *page, int authorized)
{
    int private;
    ...
    wiki_show_header(res, page, TRUE, authorized);

    html_clean_wikitext = util_htmlize(wikitext, strlen(wikitext));

    private=wiki_print_data_as_html(res, html_clean_wikitext, authorized, page);      
...
wiki_show_edit_page(HttpResponse *res, char *wikitext, char *page,
                    int preview, int authorized)
{

    wiki_show_header(res, page, FALSE, authorized);
    ...
    if (preview)
    {
        ...
        wiki_print_data_as_html(res, html_clean_wikitext, authorized, page);
    }
...
void
wiki_show_header(HttpResponse *res, char *page_title, int want_edit, int authorized)
{
    http_response_printf(res, WIKIHEADER, page_title, CssData);

    if (want_edit)
        http_response_printf(res, EditHeader, page_title, page_title, 
                             page_title, authorized ? "You are logged in":"Login");
    else
...
wiki_handle_http_request(HttpRequest *req)
{
    ...
    char         *wikitext = NULL;
    int          return_code=0, private=1, authorized=0;
    char         *msg,*link;
    char         *listbox;
    char         *value;
    char         *str_ptr;
    ...
    if ( (str_ptr=strrchr(page, '.')) ) 
    {
            ...
            !strcasecmp(str_ptr, ".pdf") )
        {
            ...
            char *ctype;
            ctype = strcat("image/",str_ptr);
            http_response_send_smallfile(res, page+1, ctype, 1000000);
                ...
                /* code < 1 reports error */
            {
                switch(return_code) {
                    case 0  : 
                        ...
                        break;
                    case -1  : 
                        msg=strdup("Name or password is too short. 4 chars or more required.");
    ...
    {
        if ( (authorized=wikilogin_getpermission(http_request_get_ip_src(req))) )
        {
            ...
            wiki_page_forbidden(res); //err msg and exit
    }
    else
    {
        /* grant permission */
        authorized=wikilogin_getpermission(http_request_get_ip_src(req));
...
/* Initializes (read or write) wiki templates */
void
wiki_init_templates()
{
    PageHeader = wiki_init_template (TEMPLATESFOLDER"/pageheader", PageHeader);
    PageFooter = wiki_init_template (TEMPLATESFOLDER"/pagefooter", PageFooter);
    ShortHelp = wiki_init_template (TEMPLATESFOLDER"/shorthelp", ShortHelp);
    LoginForm = wiki_init_template (TEMPLATESFOLDER"/loginform", LoginForm);
    NewLoginForm = wiki_init_template (TEMPLATESFOLDER"/newloginform", NewLoginForm);
    ChgPwdForm = wiki_init_template (TEMPLATESFOLDER"/chgpwdform", ChgPwdForm);
    LogInfo = wiki_init_template (TEMPLATESFOLDER"/loginfo", LogInfo);
    CreateForm = wiki_init_template (TEMPLATESFOLDER"/createform", CreateForm);
    EditHeader = wiki_init_template (TEMPLATESFOLDER"/editheader", EditHeader);
    EditForm = wiki_init_template (TEMPLATESFOLDER"/editform", EditForm);
    EditPreview = wiki_init_template (TEMPLATESFOLDER"/editpreview", EditPreview);
    DeleteForm = wiki_init_template (TEMPLATESFOLDER"/deleteform", DeleteForm);
}
...
wiki_init(char *didiwiki_home, int restore_WikiHelp)
{
    ...
    /* Check if datadir/newuser exists and create if not */
    if (stat(NEWUSERFOLDER, &st) != 0 )
    {
        if (mkdir(NEWUSERFOLDER, 0755) == -1)
        {
            fprintf(stderr, "Unable to create '%s', giving up.\n",NEWUSERFOLDER);
            exit(1);
        }
    ...
    /* Check if datadir/newuser exists and create if not */
    if (stat(TEMPLATESFOLDER, &st) != 0 )
    {
        if (mkdir(TEMPLATESFOLDER, 0755) == -1)
        {
            fprintf(stderr, "Unable to create '%s', giving up.\n",TEMPLATESFOLDER);
            exit(1);
        }
